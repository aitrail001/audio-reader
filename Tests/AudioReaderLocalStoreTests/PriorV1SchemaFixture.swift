enum PriorV1SchemaFixture {
    /// Exact v1 schema observed in shipped databases before local asset metadata
    /// and assistant provenance columns were added without a version increment.
    static let sql = """
    ...
    CREATE TABLE local_sync_asset_manifests (
    ...
    CREATE TABLE local_book_tombstones (
    ...
    CREATE TABLE local_assets (
      ...
      local_media_key TEXT NOT NULL, created_at REAL NOT NULL, updated_at REAL NOT NULL,
      server_version INTEGER NOT NULL DEFAULT 0, deleted_at REAL, last_mutation_id TEXT,
    ...
    CREATE TABLE local_transcript_revisions (
    ...
    CREATE TABLE local_transcript_overlays (
    ...
    CREATE TABLE local_transcript_overlay_conflicts (
    ...
    CREATE TABLE local_vocabulary_occurrences (
    ...
    CREATE TABLE local_assistant_results (
      id TEXT PRIMARY KEY, kind TEXT NOT NULL, status TEXT NOT NULL, language TEXT NOT NULL,
      model TEXT NOT NULL, book_id TEXT, book_title TEXT, chapter_id TEXT, chapter_title TEXT,
    ...
    CREATE TABLE local_assistant_result_history (
    ...
    CREATE TABLE local_translation_checkpoints (
    ...
    CREATE TABLE local_study_activity (
      day TEXT PRIMARY KEY, created_at REAL NOT NULL
    );
    CREATE TABLE local_reader_progress (
    ...
    CREATE INDEX idx_local_chapters_book ON local_chapters(book_id, position);
    CREATE INDEX idx_local_assets_book ON local_assets(book_id, kind);
    CREATE INDEX idx_local_sync_assets_kind ON local_sync_asset_manifests(kind, book_id, chapter_id);
    CREATE INDEX idx_local_transcript_revision_chapter ON local_transcript_revisions(chapter_id, is_active);
    CREATE INDEX idx_local_transcript_segment_range ON local_transcript_segments(chapter_id, revision_id, sequence);
    CREATE INDEX idx_local_transcript_segment_time ON local_transcript_segments(chapter_id, start_time, end_time);
    CREATE INDEX idx_local_overlays_chapter ON local_transcript_overlays(chapter_id, segment_id);
    CREATE INDEX idx_local_overlay_conflicts_segment ON local_transcript_overlay_conflicts(chapter_id, segment_id, updated_at);
    CREATE INDEX idx_local_vocab_book ON local_vocabulary_occurrences(book_id, chapter_id);
    CREATE INDEX idx_local_review_cards_vocab ON local_review_cards(vocabulary_id);
    CREATE INDEX idx_local_review_events_vocab ON local_review_events(vocabulary_id, reviewed_at);
    CREATE INDEX idx_local_assistant_chapter ON local_assistant_results(chapter_id, kind, status);
    CREATE INDEX idx_local_assistant_history_result ON local_assistant_result_history(result_id, sequence);
    CREATE INDEX idx_local_reader_progress_book ON local_reader_progress(book_id, is_current, updated_at);
    CREATE UNIQUE INDEX idx_local_reader_progress_current
      ON local_reader_progress(book_id) WHERE is_current = 1;
