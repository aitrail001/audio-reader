...
struct PlatformParityContractTests {
    ...
    func providerCredentialsUseEncryptedVault() throws {
        ...
        #expect(credentials.contains("SecItemUpdate"))
        #expect(credentials.contains("errSecDuplicateItem"))
        ...
        #expect(credentials.contains("var isSessionCached: Bool"))
        #expect(credentials.contains("loadValuesForUpdate"))
        #expect(credentials.contains("wrappingKeyForUpdate"))
    ...
    func sharesEbookOnlyImportAndAddAudio() throws {
        ...
        #expect(scanner.contains("EPUBParser.structure(from:"))
        #expect(scanner.contains("#ebook#"))
        #expect(playerView.contains("Audiobook missing"))
        #expect(playerView.contains("private struct EbookCoverPage: View"))
        #expect(playerView.contains("Cover of"))
        ...
        #expect(appleBooks.contains("AppleBooksLocalLibrary.list"))
        #expect(appleBooks.contains("kind: .ebook"))
        #expect(appleBooks.contains("kind: .audiobook"))
        #expect(appleBooks.contains("audiobookItem(from:"))
        #expect(appleBooks.contains("catch"))
        #expect(appleBooks.contains("EPUBParser.metadata(from:"))
        #expect(appleBooks.contains("EPUBParser.coverImage(from:"))
        #expect(!appleBooks.contains("EPUBParser.structure(from:"))
        #expect(!appleBooks.contains("Downloaded Apple Books titles could not be read"))
        let parser = try source("Sources/AudioReader/EPUBParser.swift")
        #expect(parser.contains("static func isPackage(at"))
        #expect(parser.contains("static func packageFile(from"))
        #expect(parser.contains("static func structure(from"))
        #expect(parser.contains("static func coverImage(from"))
        #expect(parser.contains("static func metadata(from"))
        let catalog = try source("Sources/AudioReader/AppleBooksEbookCatalog.swift")
        #expect(catalog.contains("BKBookType"))
        #expect(catalog.contains("itemName"))
        #expect(catalog.contains("AppleBooksLocalLibrary"))
        #expect(catalog.contains("case audiobook"))
        #expect(scanner.contains("EPUBParser.metadata(from:"))
