...
struct AccountSessionFlowTests {
    ...
    func accountSessionIsSharedAcrossPlatforms() throws {
        ...
        let account = try source("Sources/AudioReaderNetworking/AccountSession.swift")
        ...
        #expect(appState.contains("sentenceTranslationBatchSize"))
        ...
        #expect(live.contains("prefersEphemeralWebBrowserSession = false"))
        #expect(live.contains("callbackURLScheme"))
        #expect(live.contains("nonisolated static func makeSession"))
        #expect(try source("Sources/AudioReader/AudioReaderApp.swift").contains("handleOpenURL(url)"))
        #expect(appState.contains("func handleOpenURL(_ url: URL)"))
        #expect(appState.contains("account.completeOAuth(callbackURL: url)"))
        #expect(account.contains("code == \"not_found\""))
        #expect(account.contains("sync_asset_hydration_skipped"))
        #expect(account.contains("$0.id.caseInsensitiveCompare(currentID)"))
        #expect(account.contains("if mode.isSignedIn { return }"))
