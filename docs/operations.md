...
| Hosted Qwen | Worker uses the QwenCloud Singapore token-plan endpoint (`token-plan.ap-southeast-1.maas.aliyuncs.com`) with `QWEN_API_KEY` (from repo-root `.env.local` `qwen-key`). Native **Managed Qwen (account)** in Settings sends product tasks to `/v1/ai/*`. Exact translations and chapter summaries use HMAC-SHA256 shared cache keys (`CACHE_HMAC_SECRET`) with single-flight generation and `user_assistant_results` wrappers. Chat is private and not cached. Daily quotas apply. |
