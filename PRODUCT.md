...
AudioReader combines audiobook playback, on-device transcription with word timestamps, ebook alignment, repairable sentence overlays, dictionary lookup, vocabulary capture, first-party spaced learning and statistics, optional local Anki export, exact resume, and contextual LLM assistance. Success means readers can pair audio and EPUB, recover from damaged imports or transcript mistakes, move between devices without ambiguous progress, and move between listening and study without losing place or context.

The operator console succeeds when a signed-in admin can change live Qwen, storage, and Turnstile configuration; act on users, policies, jobs, cache, metrics, and audit events without a Worker redeploy; and understand privacy-preserving adoption, learning behavior, trends, and anomalies without seeing reading text or direct user identifiers.
...
Native reader: sidebar-owned Library, Now Reading, Words, and Settings; paired import and repair; exact Continue; phase-, entity-, batch-, item-, pending-, and conflict-aware sync with Sync now; one immutable transcript plus one validated correction overlay per sentence; Listen First sentence-boundary reveal with a bounded heard-only AI Quick Quiz and deterministic local fallback; and due-first Words with shared Today/Vocabulary subpages, a visible daily card queue, a daily new-card limit, transactional review scheduling/history, retention, due forecast, per-book progress, and optional local TSV/M4A ZIP export. Every presented transcript consumer resolves through the same overlay contract. Corrections are sentence-level and do not claim forced word alignment. Audio, EPUB, credentials, and Anki clips remain local.

Admin API: global health, runtime config, users and access, privacy requests, LLM policies and cache, jobs, feature flags, quotas, metrics, privacy-minimized product analytics, activity, request trace, and audit. Analytics covers active learners/devices, coarse server-derived country/region, language, level, platform/version, opaque content, feature, outcome, time trends, and deterministic anomaly indicators. It rejects precise location, raw IP, email, reading text, titles, authors, vocabulary, arbitrary properties, and client-spoofed server dimensions; small learner cohorts are grouped. The Operator console opens on an incident-first Desk, loads destinations independently, preserves stale panels through refresh errors, persists destinations and filters in the URL, and follows actions by request ID. High-impact mutations use a contextual before/after reason and explicit confirmation. Native apps bootstrap flags and quotas, default to the production Worker URL, and keep iPad playback in the background. Managed Qwen translations, sentence batches, and chapter summaries reuse the shared exact-content cache before calling Qwen; cache hits and lookup-only misses do not consume daily quota. Word lookups stay on-demand and are keyed with the containing resolved sentence. Microsoft Azure sign-in remains disabled until an Azure app is configured. GCS requires a GCP billing account; without it, object storage uses the existing Supabase Storage bucket.

Managed Qwen Policies separates editable operator instructions from enforced task contracts and shows the fully rendered system/user messages plus output schema for sentence, word, chapter batch, summary, chat, and bounded heard-passage quizzes. Drafts are validated before save, prompt-affecting changes disclose cache invalidation, and correlated probes return their request ID and parsed/schema result.
