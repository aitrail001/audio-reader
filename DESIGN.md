...
**Words and learning.** Words uses the same two-page workspace on macOS and iPadOS. `Today` opens first and owns the visible study action, a bounded preview of the session cards, due/new/learning/reviewed-today metrics, streak, retention, due forecast, and per-book progress in one vertically scrollable learning flow. `Vocabulary` owns search, book/list/category filters, list management, paginated rich cards, and export. The primary terracotta action starts due learning cards, due mature reviews, then at most 20 new cards. Calendar, not flame, represents study continuity; there are no points, leagues, or celebratory motion. Review advancement is visible only after schedule and immutable history persist together.
...
**Managed prompt inspection.** Policies labels editable system instructions and user templates separately from the read-only task contract. A task selector previews sentence, word, chapter batch, summary, chat, or a bounded heard-passage quiz through the same server assembler used at runtime. Effective messages and the server-owned output schema are collapsed code regions; validation status, before/after changes, cache impact, parsed probe output, and a copyable request ID remain visible without exposing secrets or real reading text.

**Per-user progress.** A user detail may show consented aggregate sync, reading, review, vocabulary, and AI-use measures plus device and activity deep links. It never renders generic sync payloads or content-bearing fields. The learner-controlled analytics preference defaults off; reads are role-scoped, audited, retained for 90 days, and suppressed entirely beyond operational sync status when consent is absent.
...
### Listen First coach (signature)
After a concealed sentence finishes, reveal that sentence and one flat `gold-soft` coach strip above playback. Keep the actions compact and native: Replay, Explain, Shadow, Quick Quiz, Continue. The strip never calls AI automatically, never displays future text, and never becomes a dashboard card stack. On iPad every action retains a 44 pt target; at narrower widths the same actions wrap vertically through `ViewThatFits`.
