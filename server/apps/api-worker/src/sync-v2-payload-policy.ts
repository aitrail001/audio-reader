...
function validateValue(
  ...
  if (Array.isArray(value)) {
    if (value.length > MAX_ARRAY_ITEMS)
      return { field: path, message: "array has too many items." };
...
function validateNestedObject(
  ...
  for (const [key, nested] of Object.entries(value)) {
    ...
    if (schema !== undefined && !schema.has(key)) {
      return {
        field: `${path}.${key}`,
        message: "nested field is not allowed by this entity schema.",
      };
    }
...
function set(...keys: string[]): ReadonlySet<string> {
  return new Set(keys);
}
