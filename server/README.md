...
CORS origins come from `CORS_ALLOWED_ORIGINS` and `ADMIN_ORIGIN`. The `local` environment also allows localhost admin (`5173`) and worker (`8787`) origins. A hosted Operator must be built with `VITE_API_BASE_URL` set to its environment's Worker URL, and every exact Pages preview alias used for staging login must be present in that Worker's CORS allowlist. JSON writes require `Content-Type: application/json` and are capped at `MAX_BODY_BYTES` (default 1 MiB). `/v1/sync/push` alone accepts up to 3 MiB; the native client caps a request at 100 mutations and keeps complete envelopes below 2.625 MiB so the largest measured transcript fits alone while count-heavy and mixed batches stay below the Worker resource threshold. Sync pulls use a service-role database RPC capped by both row count and 1 MiB of encoded payload before the response enters the Worker. Health paths accept `GET` and `HEAD`; other methods return `405` with `Allow: GET, HEAD` before body-content rules.
...
pnpm test:e2e-local # HTTP client; requires dev:api or dev:docker to be running
```
